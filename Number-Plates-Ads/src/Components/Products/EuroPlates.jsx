...
export const EuroPlates = () => {
    const [EuroPlates, setEuroPlates] = useState([]);
    
        useEffect(()=> {
            fetch("http://127.0.0.1:8000/api/euro-plates/")
                .then((res) => res.json())
                .then((data) => setEuroPlates(data))
        }, [])
  return (
    <div className='flex flex-col justify-center  bg-white'>
            <div className='w-full p-6 m-3'>
                <h2 className='text-2xl font-semibold text-center'>Euro Plates</h2>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 m-4 p-4'>
                {EuroPlates.slice(0,3).map((product) => (
